...
def stack_y_batch(data, offsets, block_size):
    """Stack per-offset Y windows into a 2D (B, block_size) matrix."""
    out = []

    for i in offsets:
        out.append(slice_y_at_offset(data, i, block_size))
...
def initialize_w_random(vocab_size, rng):
    ...
    W = rng.standard_normal((vocab_size, vocab_size), dtype=np.float64)
    return W
    pass
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    data = make_2d_zeros(len(ids), vocab_size)
...
def derive_dlogits_on_paper():
    return (
        ...
        "L = -(1/B) * sum(y * log(probs))\n\n"
        "where probs = softmax(logits).\n\n"
        "Using the derivative of softmax combined with cross-entropy,\n"
        "the gradient for each sample simplifies to:\n"
        "dL/dlogits = probs - onehot(targets).\n\n"
        "Since the loss is averaged over a batch of size B,\n"
...
def train_neural_bigram_loop(w, data, block_size, batch_size, learning_rate, num_steps, log_every):
    ...
    for step in range(num_steps):
        ...
        # Flatten (batch_size, block_size) into a single stream
        ids = X.reshape(-1)
        targets = Y.reshape(-1)
